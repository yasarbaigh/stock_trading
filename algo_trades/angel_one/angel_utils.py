...
from angel_one.configs import ALLOWED_ORDER_STATUS, ANGEL_1_BSE, ANGEL_1_NSE, STOP_CHECK, STOPPED, TYPE_SELL
...
def prepare_option_name(broker_exchange, prefix, price, date_part=None, previous_initiate=False):
    ...
    if ANGEL_1_NSE == broker_exchange:
        ...
        return [prefix.format(date_part, price, 'CE').upper(),
...
def check_and_place_order(conn, token_key, token_v, price, my_order_tag, date_part=None, previous_initiate=False):
    ...
    if token_key == SENSEX:
        option_names = prepare_option_name(ANGEL_1_BSE, token_v.get('prefix'), price, date_part, previous_initiate)
    else:
        option_names = prepare_option_name(ANGEL_1_NSE, token_v.get('prefix'), price, date_part, previous_initiate)
